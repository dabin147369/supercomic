...
public abstract class BaseProtocal<T> {
    ...
    private T getDataFromLocal(int index) throws Exception {
        ...
        try {
            ...
            if (System.currentTimeMillis() > (time + ConstantValue.REFRESH_DELAY)) {
                // 数据过期，无效
                return null;
            }
    ...
    private T getDataFromNet(int index) throws Exception {
        ...
        String url = ConstantValue.SERVER_URL + getInterfaceKey();
        RequestParams params = new RequestParams();

        params.addQueryStringParameter("index", index + "");
        ResponseStream stream = utils.sendSync(HttpRequest.HttpMethod.GET, url, params);
